...
vector<string> msaStar(const vector<string>& sequences) {
    ...
    // Align all sequences relative to the central sequence
...
vector<string> readSequencesFromFile(const string& filename) {
...
int main() {
    // Example sequences
    vector<string> sequences = {"ATTGCCATT", "ATGGCCATT", "ATCCAATTTT", "ATCTTCTT", "ACTGACC"};

    cout << "Alignment of example sequences:" << endl;
    auto alignment = msaStar(sequences);
    for (const auto& seq : alignment) {
        cout << seq << endl;
    }
    int totalScore = calculateTotalScore(alignment);
    cout << "Total score: " << totalScore << endl << endl;

    // BRCA1 sequences
    vector<string> brca1Sequences = readSequencesFromFile("C:/Users/harry/Documents/Clion_projects/test/BRCA1.txt");

    cout << "Alignment of BRCA1 sequences:" << endl;
    auto brca1Alignment = msaStar(brca1Sequences);
    for (const auto& seq : brca1Alignment) {
        cout << seq << endl;
    }
    int brca1Score = calculateTotalScore(brca1Alignment);
    cout << "Total BRCA1 score: " << brca1Score << endl;
